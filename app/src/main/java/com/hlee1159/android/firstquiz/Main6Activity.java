...
public class Main6Activity extends GroundActivity {
    ...
    public void onBackPressed() {
        ...
        builder.setPositiveButton("영웅 단계로 돌아가기", new DialogInterface.OnClickListener() {
            @Override
    ...
    public void init() {
        questions = new String[]{"ㅂ□□ㄱ", "□□ㅂ", "□ㄷㄷ□", "ㅇㄱ□ㅇ□ㅈ", "□□ㅋ", "ㅁ□□", "ㅁ□□ㅇ", "", "ㄸ□ㄱ", "□ㅈ□ㄴ"};
        answers = new String[]{"배꼽시계", "도깨비", "군더더기", "일거수일투족", "오랑캐", "모퉁이", "마수걸이", "", "띠동갑", "어제오늘"};
        hint1 = new String[]{"식사시간", "방망이", "사족", "동작", "야만", "구부러짐","", "", "12살", "최근"};
        hint2 = new String[]{"배고픔", "뿔", "혹", "하나하나", "난리","구석", "--", "", "--", "며칠 사이"};
        hint3 = new String[]{"꼬르륵", "귀신", "~가 많다", "", "이민족", "~를 돌다", "~ 홈런", "", "같다", "내일"};
    ...
    public void endOfTheGame() {
        ...
        end.setMessage("당신은 천재입니다!" + "\n다음엔 더 어렵고 더 재밌는 문제로 찾아뵙겠습니다.");
        end.setPositiveButton("나가기", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                //if user pressed "yes", then he is allowed to exit from application
                answerList.add(0);
        ...
        end.setNegativeButton("이 페이지에 머무르기", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                //if user select "No", just cancel this dialog and continue with app
                dialog.cancel();
